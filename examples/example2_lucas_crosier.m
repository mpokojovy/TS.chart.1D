%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         LUCAS & CROSIER OUT-OF-CONTROL DATASET EXAMPLE: COMPARISON AMONG VARIOUS CHARTS          %
%                                                                                                  %
...
% Load up the dataset
run 'datasets/dataset_lucas_crosier_02.m';
...
if ~isempty(IND)
    ...
    text(IND + 0.25, TS_SIGNALS(IND)/6, ['1st restart time = ', num2str(IND)], 'interpreter', 'latex', 'FontSize', 18);
end

ax = gca;
ax.XTick = sort(union([1:1:n], IND));
...
I1 = min(find(CUSUM_L_HS < -h_HS));
I2 = min(find(CUSUM_U_HS >  h_HS));

if ~isempty(I1)
    if isempty(I2)
        plot([I1 I1], [-40 CUSUM_L_HS(I1)], 'k--', I1, -40, 'k*');
        text(I1 + 0.5, CUSUM_L_HS(I1)/2, ['1st restart time = ', num2str(I1)], 'interpreter', 'latex', 'FontSize', 18);
    elseif (I1 < I2)
        plot([I1 I1], [-40 CUSUM_L_HS(I1)], 'k--', I1, -40, 'k*');
        text(I1 + 0.5, CUSUM_L_HS(I1)/2, ['1st restart time = ', num2str(I1)], 'interpreter', 'latex', 'FontSize', 18);
    end
...
if ~isempty(I2)
    if isempty(I1)
        plot([I2 I2], [-40 CUSUM_U_HS(I2)], 'k--', I2, -40, 'k*');
        text(I2 + 0.25, CUSUM_U_HS(I2)/2, ['1st restart time = ', num2str(I2)], 'interpreter', 'latex', 'FontSize', 18);
    elseif (I1 < I2)
        plot([I2 I2], [-40 CUSUM_U_HS(I2)], 'k--', I2, -40, 'k*');
        text(I2 + 0.25, CUSUM_U_HS(I1)/2, ['1st restart time = ', num2str(I2)], 'interpreter', 'latex', 'FontSize', 18);
    end
end
...
ax = gca;
ax.XTick = sort(union(union([1:1:n], I1), I2));
...
% CUSUM head start detection time
I1 = min(find(CUSUM_L_NO_HS < -h_HS));
I2 = min(find(CUSUM_U_NO_HS >  h_HS));

if ~isempty(I1)
    if isempty(I2)
        plot([I1 I1], [-40 CUSUM_L_NO_HS(I1)], 'k--', I1, -40, 'k*');
        text(I1 - 3.5, CUSUM_L_NO_HS(I1)/4, ['1st restart time = ', num2str(I1)], 'interpreter', 'latex', 'FontSize', 18);
    elseif (I1 < I2)
        plot([I1 I1], [-40 CUSUM_L_NO_HS(I1)], 'k--', I1, -40, 'k*');
        text(I1 - 3.5, CUSUM_L_NO_HS(I1)/4, ['1st restart time = ', num2str(I1)], 'interpreter', 'latex', 'FontSize', 18);
    end
...
if ~isempty(I2)
    if isempty(I1)
        plot([I2 I2], [-40 CUSUM_U_NO_HS(I2)], 'k--', I2, -40, 'k*');
        text(I2 - 3.5, CUSUM_U_NO_HS(I2)/4, ['1st restart time = ', num2str(I2)], 'interpreter', 'latex', 'FontSize', 18);
    elseif (I1 < I2)
        plot([I2 I2], [-40 CUSUM_U_NO_HS(I2)], 'k--', I2, -40, 'k*');
        text(I2 - 3.5, CUSUM_U_NO_HS(I1)/4, ['1st restart time = ', num2str(I2)], 'interpreter', 'latex', 'FontSize', 18);
    end
end
...
ax = gca;
ax.XTick = sort(union(union([1:3:19], I1), I2));
